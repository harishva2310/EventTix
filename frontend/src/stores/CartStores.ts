...
interface CartStore {
  ...
  addItem: (ticket: TicketResponse, eventDetails: EventResponse, sectionDetails: SectionModel) => void
...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  addItem: (ticket: TicketResponse, eventDetails: EventResponse, sectionDetails: SectionModel) => {
    set((state) => {
      const newItems = {
        ...state.items,
        [ticket.ticketId]: { ticket, eventDetails, sectionDetails }
      }
      return {
        items: newItems,
        total: Object.values(newItems).reduce((sum, item) => sum + item.ticket.ticketPrice, 0)
      }
    })
  },
        ...
        if (verificationResponse.data.status === 'succeeded') {
            ...
            const bookingPromises = cartTickets.map(async ([ticketId, item]) => {
                const qrData = JSON.stringify({
                    ticketId,
                    eventId: item.eventDetails?.event.event_id,
